...
router.get('/my-tickets', authenticateToken, async (req, res) => {
  try {
    ...
    const [tickets] = await conn.execute(`
      ...
      LEFT JOIN users scanner ON t.checked_in_by = scanner.id
      WHERE t.user_id = ?
...
router.post('/scan-ticket', authenticateToken, async (req, res) => {
  try {
    ...
    const [verifyTicket] = await conn.execute(
      'SELECT id, ticket_code, status, used_at as scanned_at, checked_in_by as scanned_by FROM tickets WHERE id = ?',
      [ticket.id]
    );
...
router.get('/scanned-tickets/:eventId', authenticateToken, async (req, res) => {
  try {
    ...
    const [allTickets] = await conn.execute(`
      SELECT id, ticket_code, status, used_at as scanned_at 
      FROM tickets 
      WHERE event_id = ?
    `, [eventId]);
  ...
  try {
    ...
    try {
      ...
      if (ticket.ticket_status === 'scanned' || ticket.ticket_status === 'used') {
        ...
        // Emit duplicate scan alert via WebSocket
